...
# ⚠️ SUPPRESSION DU TRUNCATE ICI ⚠️
# (il sera mis plus bas dans main())
...
def generate_orders(conn, n=5000):
    ...
    for i in range(n):
        ...
        user_id, country = random.choice(users)
        ...
        for product_id, unit_price in order_products:
...
def generate_events(conn, n=10000):
    ...
    for i in range(n):
        ...
        events.append((user_id, event_type, event_ts, metadata))
...
def main():
    ...
    cursor.execute("TRUNCATE TABLE crm_contacts RESTART IDENTITY CASCADE;")
    conn.commit()
    ...
    generate_events(conn, 10000)
    generate_crm_contacts(conn, 500)
